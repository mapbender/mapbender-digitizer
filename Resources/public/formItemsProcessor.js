var FormItemsProcessor = function (schema) {
...
FormItemsProcessor.prototype = {
    ...
    processSelectWithData: function(item) {
        var dataManagerUtils = this;
        var schema = dataManagerUtils.schema;
        var widget = schema.widget;
    ...
    processFile: function(item,feature) {
        var dataManagerUtils = this;
        var schema = dataManagerUtils.schema;
        var widget = schema.widget;
    ...
    processImage: function(item,feature) {
        var dataManagerUtils = this;
        var schema = dataManagerUtils.schema;
        var widget = schema.widget;
        var itemAugmenter = {};
        ...
        if (!item.origSrc) {
            itemAugmenter.origSrc = item.src;
        }
    ...
    processCurrentFormItemsWithDataManager: function (feature) {
        var dataManagerUtils = this;
        ...
        DataUtil.eachItem(schema.formItems, function (item) {
            ...
            if (item.type === "resultTable" && item.editable && !item.isProcessed) {
                item = dataManagerUtils.processResultTable(item);
            }
            ...
            if (item.type === "select" && !item.isProcessed && ((item.dataStore && item.dataStore.editable && item.dataStore.popupItems) || item.dataManagerLink)) {
                item = dataManagerUtils.processSelectWithData(item);
            }

            if (item.type === "file") {
                item = dataManagerUtils.processFile(item,feature);
            }

            if (item.type === 'image') {

                item = dataManagerUtils.processImage(item,feature);
     ...
     * @private
     */
    openEditDialog: function (dataItem, formItems, schema, ref) {
