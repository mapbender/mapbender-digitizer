        ...
        clone: function () {
            ...
            for (var property in formItem) {
                if (formItem.hasOwnProperty(property)) {  // copy only
                    ...
                    if (typeof value == "object" &&!Array.isArray(value)  && property !== 'schema' && property !== "columns") {
                        clonedItem[property] = $.extend(true, {}, value);
                    } else {
        ...
        openEditDialog: function (data,item, ref) {
            ...
            DataUtil.eachItem(schema.formItems, function (item) {
                ...
                if (item.type === 'image') {
                    ...
                    if (item.relative) {
                        item.src = src.match(/^(http[s]?:|\/{2})/) ? src : Mapbender.Digitizer.Utilities.getAssetsPath(src);
                    } else {
                        item.src = src;
                    }
                }
